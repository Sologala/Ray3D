...
def init_config(args, data_config, model_config, train_config, plot_config):
    ...
    if args.timestamp == "":
        ...
        train_config["CHECKPOINT"] = os.path.join(
            ...
            "{}_{}_{}_FRAME{}_LR{}_EPOCH{}_BATCH{}_{}".format(
                train_config["CATEGORY"],
                data_config["DATASET"],
                model_config["MODEL"],
        ...
        all_exps = [
            d
            for d in os.listdir(train_config["CHECKPOINT"])
            if os.path.isdir(os.path.join(train_config["CHECKPOINT"], d))
        ]
    ...
    # use historical model to evaluating or model training resumeing
    if args.evaluate != "" or args.resume != "":
        ...
        save_str = (
            "rsync -au  --exclude 'data/' --exclude 'checkpoint/' --include '*/' --include '*.py' --exclude '*' . "
        )
