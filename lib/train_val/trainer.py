...
class Trainer:
    ...
    def train(self, epoch, mlog):
        ...
        for _, batch_3d, batch_2d in self.train_generator.next_epoch():
            ...
            if self.model_config["BONE_COMPARISON"]:
                ...
                epoch_loss_3d_bone += (
                    inputs_3d.shape[0] * inputs_3d.shape[1] * (loss_3d_bone_length.item() + loss_3d_bone_angle.item())
                )
            ...
            for cam, batch, batch_2d in self.test_generator.next_epoch():
                ...
                if self.model_config["BONE_COMPARISON"]:
                    ...
                    epoch_loss_3d_bone += (
                        inputs_3d.shape[0]
                        * inputs_3d.shape[1]
                        * (loss_3d_bone_length.item() + loss_3d_bone_angle.item())
                    )
                ...
                if self.model_config["TRAJECTORY_MODEL"]:
                    ...
                    epoch_loss_3d_trj += (
                        inputs_3d.shape[0]
                        * inputs_3d.shape[1]
                        * weighted_mpjpe(predicted_3d_trj, inputs_traj[:, :, 0:1], w).item()
                    )
                else:
                    loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d)
        ...
        with torch.no_grad():
            ...
            for cam, batch, batch_2d in test_generator.next_epoch():
                ...
                if return_predictions:
                    if self.model_config["TRAJECTORY_MODEL"]:
                        return (
                            (self.pos_model_test(inputs_2d) + self.trj_model_test(inputs_2d)).squeeze(0).cpu().numpy()
                        )
                    else:
                ...
                if self.model_config["TRAJECTORY_MODEL"]:
                    ...
                    if flip_test:
                        ...
                        predicted_3d_trj = torch.mean(
                            torch.cat((predicted_3d_trj, predicted_3d_trj_flip), dim=1), dim=1, keepdim=True
                        )
                    predicted_3d_pos += predicted_3d_trj
                    if not cam is None:
                ...
                epoch_loss_3d_root += (
                    inputs_3d.shape[0]
                    * inputs_3d.shape[1]
                    * mpjpe(predicted_3d_pos[:, :, 0:1, :], inputs_3d[:, :, 0:1, :]).item()
                )
        ...
        if "CAMERA_WISE_PERFORMANCE" in self.data_config and self.data_config["CAMERA_WISE_PERFORMANCE"]:
            ...
            for cam_idx in range(len(camera_dist)):
                ...
                for action_key in all_actions.keys():
                    ...
                    poses_cam, poses_act, poses_2d_act = pose_data.fetch_via_action(
                        all_actions[action_key], camera_idx=cam_idx
                    )
                    action_generator = UnchunkedGenerator(
                        poses_cam,
                        poses_act,
                        poses_2d_act,
                        pad=pad,
                    ...
                    e1, e2, e3, ev, er = self.evaluate_core(
    ...
    def render(
        ...
        render_generator = UnchunkedGenerator(
            None,
            None,
            [input_keypoints],
            pad=pad,
        ...
        if (
            ...
            and not self.model_config["TRAJECTORY_MODEL"]
            and not self.data_config["RAY_ENCODING"]
        ):
            # Reapply trajectory
