...
def main():
    ...
    if not models["train_trj"] is None:
        trainable_params.append(
            {
                "name": "trj_model",
                "params": models["train_trj"].parameters(),
                "lr": train_config["LEARNING_RATE_TRAJECTORY"],
            }
        )
    ...
    if model_config["FINETUNE"] != "":
        ...
        checkpoint = torch.load(model_config["FINETUNE"], map_location=lambda storage, loc: storage)
