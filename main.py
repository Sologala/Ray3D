...
def export_onnx_model(pos_net, trj_net):
    dummy_input = torch.randn(1, 9, 17, 2)

    onnx_path = "video_pos_net.onnx"
    torch.onnx.export(
        pos_net.module.cpu(),
        dummy_input,
        onnx_path,
...
def main():
    ...
    """-------- parse arguments --------"""
    data_config, model_config, train_config, plot_config, mlog = init_config(
        args, data_config, model_config, train_config, plot_config
    )
    ...
    if train_config["OPTIMIZER"] == "AdamW":
        optimizer = optim.AdamW(trainable_params, lr=train_config["LEARNING_RATE"], weight_decay=0.1)
    elif train_config["OPTIMIZER"] == "Adam":
        optimizer = optim.Adam(trainable_params, lr=train_config["LEARNING_RATE"], amsgrad=True)
    else:
    ...
    if model_config["PRETRAIN"] != "":
        ...
        checkpoint = torch.load(model_config["PRETRAIN"], map_location=lambda storage, loc: storage)
    ...
    # export_onnx_model(models["train_pos"], models["train_trj"])
    # return 0

    best_performance = None
    if args.resume or args.evaluate:
        chk_filename = os.path.join(train_config["CHECKPOINT"], args.resume if args.resume else args.evaluate)
        ...
        while epoch <= train_config["EPOCHS"]:
            ...
            if epoch % 16 == 0:
                trainval_engine.evaluate(
                    mlog, subjects_test, pose_data, action_filter, pad, causal_shift, epoch, plot=True
                )
    ...
    if args.render:
        ...
        trainval_engine.render(
            dataset,
            keypoints,
            pose_data.keypoints_metadata,
            pad,
            causal_shift,
            kps_left,
            kps_right,
